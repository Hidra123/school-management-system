...
// -------------------------------------------------------------
function printSectionAsUi(sectionId: string, title: string, subtitle: string, pageSize: "A4" | "A3" = "A4") {
  ...
  w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8" /><title>${title}</title>
    <style>${css}
      @page { size: ${pageSize} landscape; margin: 7mm; }
...
export default function TimetablePage() {
  ...
  if (!data?.settings) return;
  printSectionAsUi("tt-print-general", "GENERAL TEACHING TIMETABLE", `GENERAL SCHOOL TEACHING TIME TABLE · ${data.settings.academicYear}`, "A3");
  }
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                  <div className="overflow-x-autooverflow-x-auto" id="tt-print-general">
                    <table className="w-full min-w-[980px] border-collapse text-xs">
                      ...
                      <tbody>
                        {DAYS.filter((d) => dayFilter === "all" || dayFilter === String(d.id)).map((day, dayIdx) => (
                          data.classes.map((c, cIdx) => {
                            ...
                            return (
                              <tr key={`${day.id}-${c.id}`} className={cIdx % 2 ? "bg-slate-50/50" : "bg-white"}>
                                ...
                                {dayIdx === 0 && isFirst && (
                                  <td
                                    rowSpan={DAYS.length * classCount}
                          ...
                          <tbody>
                            {DAYS.map((day, dayIdx) => (
                              <tr key={day.id} className="hover:bg-slate-50">
                                ...
                                {dayIdx === 0 && (
                                  <td rowSpan={DAYS.length} className="border border-slate-200 bg-amber-50 px-1 py-2 text-center align-middle"><span className="text-[9px] font-black uppercase tracking-widest text-amber-800" style={{ writingMode: "vertical-rl" }}>Break Time</span></td>
                                )}
                                ...
                                {dayIdx === 0 && (
                                  <td rowSpan={DAYS.length} className="border border-slate-200 bg-rose-50 px-1 py-2 text-center align-middle"><span className="text-[9px] font-black uppercase tracking-widest text-rose-800" style={{ writingMode: "vertical-rl" }}>Lunch Time</span></td>
                                )}
                                {([8, 9] as const).map((n) => {
                                  const slot = slotMap.get(`${day.id}-${n}-${targetClass.id}`);
                                  const disp = getSlotDisplay(slot);
...
{PERIODS.map((p) => {
                                  ...
                                  if (!slot && !special) {
                                    return (
                                      <Fragment key={p.num}>
                                        {dayIdx === 0 && (p.num === 5 || p.num === 8) && (
                                          <td rowSpan={DAYS.length} className={cls("border border-slate-200 px-1 py-2 text-center align-middle", p.num === 5 ? "bg-amber-50" : "bg-rose-50")}>
                                  ...
                                  return (
                                    <Fragment key={p.num}>
                                      ...
                                      <td className="border border-slate-200 px-2 py-2 text-center bg-indigo-50/60">
                                        ...
                                        <div className="font-bold text-indigo-700 text-[10px] mt-0.5">{slot!.subjectCode || slot!.subjectName || "Subject"}</div>
