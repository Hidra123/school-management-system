...
export default function AttendanceReportTab({
  ...
  classes: ClassRow[];
  /** When true, adds an "All Classes" option that aggregates across every visible class. */
  ...
  const canGenerate = allowAllClasses || !!classId;
  const url = canGenerate
    ? `/api/attendance/report?${classId ? `classId=${classId}&` : ""}month=${month}&year=${year}`
    : null;
    ...
    <div>
      <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-end">
          <div className="flex-1">
            ...
            <select value={classId} onChange={(e) => setClassId(e.target.value)} className={inputCls}>
              <option value="">{allowAllClasses ? "All Classes" : "--"}</option>
              {classes.map((c) => (
        ...
        {!loadedUrl ? (
          <EmptyState
            icon="🗂️"
            title={allowAllClasses ? "Select month & year" : "Select class, month & year"}
            message={allowAllClasses ? "Pick a class (or leave as All Classes) then click Generate." : "Choose a class then click Generate to see analytics."}
          />
        ) : loading ? (
