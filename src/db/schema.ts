...
export const todReports = pgTable(
  "tod_reports",
  {
    ...
    schoolName: varchar("school_name", { length: 150 })
      .notNull()
      .default("MANGI WINGIA SECONDARY SCHOOL"),
    headmasterName: varchar("headmaster_name", { length: 120 })
      .notNull()
      .default("Saidi Rashid Mpambika"),
    motto: varchar("motto", { length: 200 })
      .notNull()
      .default("NAMBAWALA SECONDARY SCHOOL: Honor All Build Together"),
    todSignature: varchar("tod_signature", { length: 100 }).notNull().default(""),
    headSignature: varchar("head_signature", { length: 100 }).notNull().default(""),
    attendanceRate: doublePrecision("attendance_rate").notNull().default(0),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
