...
export default function TimetablePage() {
  ...
  function handlePrintGeneral() {
    ...
    const totalTableRows = DAYS.length * clList.length;
    let globalRowIdx = 0;

    const daysRowsHtml = DAYS.map((day, dIdx) => {
      const dayClasses = clList.map((c, cIdx) => {
        const isVeryFirstRow = globalRowIdx === 0;
        globalRowIdx++;
        ...
        const p1_4 = [1, 2, 3, 4]
          .map((p) => {
            ...
            return `<td style="border:1px solid #94a3b8;text-align:center;padding:2px 1px;vertical-align:middle;${bg}">
              <div style="font-weight:900;font-size:10px;line-height:1.15;color:#0f172a;">${disp.label}</div>
              ${disp.sub ? `<div style="font-size:8px;font-weight:600;color:#475569;line-height:1;margin-top:2px;">${disp.sub}</div>` : ""}
            </td>`;
          })
          .join("");
        ...
        let p8_9 = "";
        if (day.id === 3 && cIdx === 0) {
          p8_9 = `<td colspan="2" rowspan="${clList.length}" style="border:1px solid #94a3b8;text-align:center;font-weight:900;font-size:13px;background:#eef2ff;color:#3730a3;letter-spacing:1.5px;vertical-align:middle;">
            RELIGIO
          </td>`;
        ...
        // Single continuous merged columns spanning ALL days and ALL classes:
        const breakCell =
          isVeryFirstRow
            ? `<td rowspan="${totalTableRows}" style="border:1px solid #94a3b8;text-align:center;vertical-align:middle;font-weight:900;font-size:11px;background:#fef3c7;color:#92400e;letter-spacing:2px;padding:4px;writing-mode:vertical-rl;transform:rotate(180deg);">
                BREAK TIME
              </td>`
            : "";
        ...
        const lunchCell =
          isVeryFirstRow
            ? `<td rowspan="${totalTableRows}" style="border:1px solid #94a3b8;text-align:center;vertical-align:middle;font-weight:900;font-size:11px;background:#ffe4e6;color:#9f1239;letter-spacing:2px;padding:4px;writing-mode:vertical-rl;transform:rotate(180deg);">
                LUNCH TIME
              </td>`
            : "";
        ...
        const assemblyCell =
          isVeryFirstRow
            ? `<td rowspan="${totalTableRows}" style="border:1px solid #94a3b8;text-align:center;vertical-align:middle;font-weight:900;font-size:10px;background:#e0e7ff;color:#3730a3;letter-spacing:1.5px;padding:3px;writing-mode:vertical-rl;transform:rotate(180deg);">
                ASSEMBLY
              </td>`
            : "";
        ...
        const extraCell =
          cIdx === 0
            ? `<td rowspan="${clList.length}" style="border:1px solid #94a3b8;text-align:center;vertical-align:middle;font-size:10px;font-weight:800;padding:4px 3px;background:#ecfdf5;color:#065f46;">
        ...
        const dayCell =
          cIdx === 0
            ? `<td rowspan="${clList.length}" style="border:1px solid #94a3b8;text-align:center;vertical-align:middle;font-weight:900;font-size:10px;background:#f8fafc;color:#0f172a;letter-spacing:1px;padding:4px 2px;text-transform:uppercase;">
                ${day.name}
              </td>`
            : "";
        ...
        return `<tr>
          ${dayCell}
          <td style="border:1px solid #94a3b8;text-align:center;padding:4px 2px;font-size:10px;font-weight:900;background:#f8fafc;color:#0f172a;">${classLabel}</td>
          ${p1_4}
          ${breakCell}
          ${p5_7}
          ${lunchCell}
    ...
    const fullHtml = `<!DOCTYPE html><html><head><meta charset="utf-8">
      ...
      <style>
        ...
        * { box-sizing: border-box; print-color-adjust: exact; -webkit-print-color-adjust: exact; }
        ...
        .hdr { text-align: center; margin-bottom: 8px; border: 1.5px solid #94a3b8; border-radius: 10px; padding: 8px 14px; background: #fff; }
        .hdr h2 { margin: 0; font-size: 12px; font-weight: 800; letter-spacing: 2px; text-transform: uppercase; color: #64748b; }
        .hdr h1 { margin: 3px 0; font-size: 24px; font-weight: 900; letter-spacing: 1px; text-transform: uppercase; color: #0f172a; }
        .hdr h3 { margin: 2px 0 6px; font-size: 15px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; color: #4338ca; }
        .chips { display: flex; justify-content: center; gap: 16px; font-size: 11px; font-weight: 700; color: #334155; }
        .chip { background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 999px; padding: 3px 12px; }
        table.tt { width: 100%; border-collapse: collapse; border: 2px solid #64748b; font-size: 9.5px; }
        table.tt th { border: 1px solid #94a3b8; text-align: center; padding: 4px 2px; vertical-align: middle; }
        table.tt td { height: 26px; }
        .notes { margin-top: 8px; font-size: 9px; line-height: 1.4; font-weight: 600; color: #334155; border: 1px solid #cbd5e1; border-radius: 8px; padding: 6px 12px; background: #f8fafc; }
        .sig { display: flex; justify-content: space-between; margin-top: 16px; font-size: 11px; color: #334155; font-weight: 700; }
        .sig div { border-top: 2px solid #64748b; width: 28%; padding-top: 4px; }
      </style>
    </head><body>
      <div class="hdr">
        <h2>${s.councilName}</h2>
  ...
  function handlePrintClass(targetClassId: number) {
    ...
    const rowsHtml = DAYS.map((day, dIdx) => {
      ...
      // Merged columns for BREAK, LUNCH, ASSEMBLY across all 5 days
      const breakCell = isFirst
        ? `<td rowspan="5" style="border:1px solid #94a3b8;text-align:center;vertical-align:middle;font-weight:900;font-size:9.5px;background:#fef3c7;color:#92400e;letter-spacing:1.5px;padding:4px;writing-mode:vertical-rl;transform:rotate(180deg);">
            BREAK TIME
          </td>`
        : "";

      const lunchCell = isFirst
        ? `<td rowspan="5" style="border:1px solid #94a3b8;text-align:center;vertical-align:middle;font-weight:900;font-size:9.5px;background:#ffe4e6;color:#9f1239;letter-spacing:1.5px;padding:4px;writing-mode:vertical-rl;transform:rotate(180deg);">
            LUNCH TIME
          </td>`
        : "";

      const assemblyCell = isFirst
        ? `<td rowspan="5" style="border:1px solid #94a3b8;text-align:center;vertical-align:middle;font-weight:900;font-size:9px;background:#e0e7ff;color:#3730a3;letter-spacing:1px;padding:3px;writing-mode:vertical-rl;transform:rotate(180deg);">
            ASSEMBLY
          </td>`
        : "";
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[980px] border-collapse text-xs">
                      ...
                      <tbody>
                        {(() => {
                          const filteredDays = DAYS.filter((d) => dayFilter === "all" || dayFilter === String(d.id));
                          const totalRows = filteredDays.length * data.classes.length;
                          let overallRow = 0;

                          return filteredDays.map((day) => (
                          data.classes.map((c, cIdx) => {
                            const isFirstInDay = cIdx === 0;
                            const isVeryFirstOverall = overallRow === 0;
                            overallRow++;
                            ...
                            return (
                              <tr key={`${day.id}-${c.id}`} className={cIdx % 2 ? "bg-slate-50/50" : "bg-white"}>
                                ...
                                {/* BREAK TIME (Single continuous merged column) */}
                                {isVeryFirstOverall && (
                                  <td
                                    rowSpan={totalRows}
                                    className="border border-slate-200 bg-amber-50 text-amber-800 text-center text-xs font-black uppercase align-middle tracking-widest"
                                  >
                                    BREAK
                                  </td>
                                )}
                                ...
                                {/* LUNCH TIME (Single continuous merged column) */}
                                {isVeryFirstOverall && (
                                  <td
                                    rowSpan={totalRows}
                                    className="border border-slate-200 bg-rose-50 text-rose-800 text-center text-xs font-black uppercase align-middle tracking-widest"
                                  >
                                    LUNCH
                                  </td>
                                )}
                                ...
                                {/* ASSEMBLY (Single continuous merged column) */}
                                {isVeryFirstOverall && (
                                  <td
                                    rowSpan={totalRows}
                                    className="border border-slate-200 bg-indigo-50 text-indigo-800 text-center text-xs font-black uppercase align-middle tracking-widest"
                                  >
                                    ASSEMBLY
                                  </td>
                                )}
                          ...
                          <tbody>
                            {DAYS.map((day, dIdx) => {
                              ...
                              <tr key={day.id} className="hover:bg-slate-50">
                                ...
                                {/* BREAK TIME (merged across all 5 days) */}
                                {isFirst && (
                                  <td
                                    rowSpan={5}
                                    className="border border-slate-200 bg-amber-50 text-amber-800 text-center text-[11px] font-black uppercase align-middle tracking-wider"
                                  >
                                    BREAK
                                  </td>
                                )}
                                ...
                                {/* LUNCH TIME (merged across all 5 days) */}
                                {isFirst && (
                                  <td
                                    rowSpan={5}
                                    className="border border-slate-200 bg-rose-50 text-rose-800 text-center text-[11px] font-black uppercase align-middle tracking-wider"
                                  >
                                    LUNCH
                                  </td>
                                )}
                                ...
                                {/* ASSEMBLY (merged across all 5 days) */}
                                {isFirst && (
                                  <td
                                    rowSpan={5}
                                    className="border border-slate-200 bg-indigo-50 text-indigo-800 text-center text-[10px] font-black uppercase align-middle tracking-wider"
                                  >
                                    ASSEMBLY
                                  </td>
                                )}
