...
/** Signature format: "Hidra Ramadhani Omari" → "H.R. Omary" (initials + full surname). */
...
export default function TodPage() {
  ...
  useEffect(() => {
    ...
    // Saved reports keep their saved text (manual mode); fresh drafts stay auto.
    setTodAuto(!d.mine?.todComment);
    setHeadAuto(!d.mine?.headComment);
  }, [dataFetch.data]);
  ...
  function openPrint(res?: ReportRow | null) {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Duty Report ${mine.date} - ${esc(mine.teacherName)}</title><style>
      ...
      * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
      ...
      <p style="border:2px solid #4f46e5;background:#eef2ff;color:#312e81;font-weight:900;font-size:12.5px;padding:6px 10px;margin:10px 0;border-radius:6px;text-align:center;">PERCENTAGE OF ATTENDANCE: PRESENT / TOTAL × 100 = <b style="color:#7c3aed;font-size:15px;">${pct}%</b></p>
      <div class="commentbox"><b style="color:#047857;">📝 T.O.D.'S COMMENT(S):</b><br/>${esc(mine.todComment)}</div>
      ...
      <div class="commentbox hos"><b style="color:#4338ca;">🛡️ HEADMASTER'S COMMENT(S):</b><br/>${esc(mine.headComment)}</div>
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
            ...
            <div className="border-t border-slate-100 p-5">
              ...
              <div className="flex items-center justify-between">
                <label className="text-[10px] font-bold uppercase tracking-wide text-emerald-600">✨ Auto-generated from attendance %</label>
                <button
                  onClick={() => { setTodComment(autoTodComment(percentage, totals.absent)); setTodAuto(true); }}
              ...
              <textarea
                rows={3}
                value={todComment}
                onChange={(e) => { setTodComment(e.target.value); setTodAuto(false); }}
                placeholder="Auto-generated from your attendance % — edit freely…"
                className={inputCls}
              />
                ...
                {isHead ? (
                  <>
                    <div className="flex items-center justify-between">
                      <label className="text-[10px] font-bold uppercase tracking-wide text-emerald-600">✨ Auto-generated from attendance %</label>
                      <button
                        onClick={() => { setHeadDraft(autoHeadComment(percentage, totals.absent)); setHeadAuto(true); }}
                        className="rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700 hover:bg-emerald-100"
                      >
                        🔄 Regenerate
                      </button>
                    ...
                    <textarea
                      rows={3}
                      value={headDraft}
                      onChange={(e) => { setHeadDraft(e.target.value); setHeadAuto(false); }}
